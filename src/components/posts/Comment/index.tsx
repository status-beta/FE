...
function Comment({ comment, refetch, groupId, commentId, setCommentCount, detailMode }: CommentProps) {
  ...
  // 모달 닫기
  const onCloseModal = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    e.stopPropagation();
  ...
  return (
    <>
      ...
      <FreeComment>
        <CommentUserImg>
          <Image src={comment.groupAvatarImg} alt="profile" onError={handleImgError} />
        </CommentUserImg>
        <CommentContainer>
          <CommentHeader>
            <CommentUserInfo>
          ...
          <CommentResponse>
            <CommentLoadTime>{formatTimeAgo(comment.createdAt)}</CommentLoadTime>
      ...
      {editMyComment ? (
        <CommentForm>
          ...
          <CommentInput onKeyDown={onKeyDown} onChange={onChangeText} value={textValue} required />
          <CommentSubmitBtn>
