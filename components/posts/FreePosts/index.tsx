...
import BoldSvg from '@/public/assets/svg/BoldSvg';
import ItalicSvg from '@/public/assets/svg/ItalicSvg';
import ASvg from '@/public/assets/svg/ASvg';
import FileSvg from '@/public/assets/svg/FileSvg';
import EmojiSvg from '@/public/assets/svg/EmojiSvg';
import PostButtonSvg from '@/public/assets/svg/PostButtonSvg';

import SearchSvg from '@/public/assets/svg/SearchSvg';
import { Buttons } from '@/components/Chats/ChatForm/styles';
import { AnimatePresence, LayoutGroup } from 'framer-motion';
import NullPostSvg from '@/public/assets/svg/NullPostSvg';
...
function FreePosts() {
  ...
  // 글작성 모달 이벤트
  const PostModalOpen = (e: React.MouseEvent) => {
    e.stopPropagation();
    //setIsForm(true);
  };
            ...
            {/* {isSuccess && getPost?.pages[0].data.length ? (
                  getPost?.pages?.map((page) => (
                    <React.Fragment key={page?.currentPage}>
                      {page?.data?.map((post) => {
                        return (
                          <FreePostItem nowRef={ref} groupId={groupId} key={post?.postId} refetch={refetch} post={post} />
                        );
                      })}
                    </React.Fragment>
                  ))
